...
#define BK_DIST_RELATIVE_PATH 0
// -- to support bk_dist accelerate, for tbs p2p
...
public:
	...
	// do not support now
	// static FString RemapPath(const FString& SourcePath);
#endif
	// -- to support bk_dist accelerate, for tbs p2p
...
#define BK_DIST_OPT_FEW_SHADER 1
#define BK_DIST_DYNAMIC_PORT 1
class FShaderCompileBKDistThreadRunnable : public FShaderCompileThreadRunnableBase
{
private:
	/**
	* A map of directory paths to shader jobs contained within that directory.
	* One entry per BK task.
	*/
	class FShaderBatch
	{
	...
	public:
		...
		const FString& InputFileSuffix;
		const FString& OutputFileSuffix;
		...
		FShaderBatch(const FString& InDirectoryBase, const FString& InInputFileSuffix, const FString& InOutputFileSuffix, const FString& InGuid, int32 InDirectoryIndex, int32 InBatchIndex);
	...
	/** The handle referring to the BK console process, if a build is in progress. */
	FProcHandle BuildProcessHandle;

	/** Process ID of the BK console, if a build is in progress. */
	uint32 BuildProcessID;

	// process id of bk-shader-tool
	int32 BKShaderToolID;

	FShaderBatch* ShaderBatching;

	/** The full path to the two working directories for BK shader builds. */
	const FString WorkingDirectory;

	int32 BatchIndex;
	int32 DirectoryIndex;
	/** uniq id for this process. */
	FString Guid;
	...
	bool BKShaderAvailable;
	int32 BKCheckFailCounter;

	bool InLaunching;

	TArray<FShaderBatch*> ShaderBatchesInFlight;
	TArray<FShaderBatch*> ShaderBatchesFull;
	TArray<FShaderBatch*> ShaderBatchesCompleted;

	void PostCompletedJobsForBatch(FShaderBatch* Batch);
	void CleanCompleted();
	void GatherResultsFromBKDist();
	int32 ObtainJobsFromMgr(TArray<FShaderCommonCompileJobPtr>& OutJobs);
	void ObtainJobs();
	void LaunchBKProcessIfNeeded();
	void CheckBKProcessStatus();
	FString GetShaderJson(const FString& WorkerName, uint32 InProcessId, TArray<FShaderBatch*>& shaders);
	void ReclaimJobs();
	bool HasJobs();

	TArray<uint8>* FormatHttpRequest(int32 port, FString message, FString url);
	FString GetHttpJson(FString& response);
	int SendMessage(FString IPStr, int32 port, FString message, FString url, FString* outret);
	// bool IsBKShaderAvailable();
	bool IsBKShaderToolPIDChanged(FString& response);
	int GetRetCode(FString& response);
	void CheckBKTool();
#if BK_DIST_DYNAMIC_PORT
	int32 getPortFromProcessInfo();
#endif
	int SendShaders(FString& OutputJson);
	void MoveFullToFlight(int32 maxsizetomove, TArray<FShaderBatch*>& flightarray, bool removefull);

public:
	/** Initialization constructor. */
	FShaderCompileBKDistThreadRunnable(class FShaderCompilingManager* InManager);
	virtual ~FShaderCompileBKDistThreadRunnable();
...
class FShaderCompilingManager : IAssetCompilingManager
{
	...
	// ++ to support bk_dist accelerate
	friend class FShaderCompileBKDistThreadRunnable;
	// --
