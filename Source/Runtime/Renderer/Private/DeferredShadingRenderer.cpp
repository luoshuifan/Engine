...
DECLARE_GPU_STAT_NAMED(GaussianSplatting, TEXT("Gaussian Splatting"));
...
void FDeferredShadingSceneRenderer::RenderGaussianSplatting(FRDGBuilder& GraphBuilder, const TArray<FViewInfo>& InViews, FSceneTextures& SceneTextures, bool bIsEarlyDepthComplete)
{
	LLM_SCOPE_BYTAG(GaussianSplatting);
	TRACE_CPUPROFILER_EVENT_SCOPE(InitGaussianRaster);
	...
	// Primary raster view
	{
		...
		GS::FRasterContext RasterContext;

		{
			RDG_EVENT_SCOPE_STAT(GraphBuilder, GaussianSplatting, "GaussianSplatting::Setup");
			RDG_GPU_STAT_SCOPE(GraphBuilder, GaussianSplatting);

			bool bVisualizeActive = false;
			bool bVisualizeOverdraw = false;

			RasterContext = GS::InitRasterContext(
				GraphBuilder,
				ViewFamily,
				RasterTextureSize,
				RasterTextureRect,
...
void FDeferredShadingSceneRenderer::Render(FRDGBuilder& GraphBuilder)
{
	...
	//Begin GS
	ViewFamily.Scene->UpdateGSSceneCard();
	const bool bGaussianSplatEnabled = true;
	//End GS
	...
	auto RenderPrepassAndVelocity = [&](auto& InViews, auto& InNaniteBasePassVisibility, auto& NaniteRasterResults, auto& PrimaryNaniteViews)
	{
		...
		//Begin GS
		{
			if(bGaussianSplatEnabled && InViews.Num() > 0)
			{
