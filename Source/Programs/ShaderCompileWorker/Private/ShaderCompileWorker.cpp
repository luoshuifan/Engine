...
public:
	...
	void Loop(FString& CrashOutputFile)
	{
        ...
        if (FString(WorkingDirectory).Equals(TEXT("\"\"")))
        {
            const_cast<FString&>(this->InputFilePath) = this->InputFilename;
            const_cast<FString&>(this->OutputFilePath) = this->OutputFilename;
        }
        // --
	...
	void VerifyFormatVersions(TMap<FString, uint32>& ReceivedFormatVersionMap)
	{
		for (auto Pair : ReceivedFormatVersionMap)
		{
			...
			if (Found)
			{
				if (Pair.Value != *Found)
				{
					// ++ to support bk_dist accelerate, try do not check version
                    UE_LOG(LogShaders, Log, TEXT("Mismatched shader version for format %s: Found version %u but expected %u; did you forget to build ShaderCompilerWorker?"), *Pair.Key, *Found, Pair.Value);
